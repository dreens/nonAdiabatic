...
%function hopgrid = nonAdClusterSwitching(signn)
...
f = figure;
cmapg = colormap(gray);
gmap = flipud(cmapg(20:end,:));
gmap = gmap(1:9:end,:);
gmap = [gmap ; 0.6 0.6 0 ; 0.8 0.8 0 ; 1 1 0];
close(f)
for i=[21:-1:1 2:21]
    hold off
    fh = figure(333);
    fh.Position = [200 400 450 370];
    if i>npz
        j = 2*npz - i;
    else
        j = i;
    ...
    full = [mm(end:-1:2,end:-1:2) pp(end:-1:2,:) ; ...
            pp(:,end:-1:2) mm];
    contourf(xxx,yyy,log10(full),-8:0,'LineStyle','none')
    hold on
    exp = Exp(:,:,j);
    eyp = Eyp(:,:,j);
    exq = Exq(:,:,j);
    ...
    title({sprintf('Switching Spin-Flips, \\phi=%3d^\\circ',phi),'\color[rgb]{0 .85 .2}Green = E-field Before Switch, \color{red}Red = After'},'FontSize',14)
    hcb = colorbar;
    ylabel(hcb,'log10 Hopping Probability','FontSize',13)
    
    h = [h,getframe(gcf)];
end

%% Write a Video
a = VideoWriter('switchLoss.avi');
a.FrameRate = 3;
open(a);
writeVideo(a,h);
close(a);